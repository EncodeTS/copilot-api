...
const streamOpenAICompatibleProviderMessages = ({
  ...
  debugJson(logger, "provider.messages.openai_compatible.streaming", {
    provider,
  })
  return streamSSE(c, async (stream) => {
...
const respondOpenAICompatibleProviderMessagesJson = (
  ...
  const { body, provider } = options
  const anthropicResponse = translateToAnthropic(body)
  debugJson(logger, "provider.messages.openai_compatible.no_stream result:", {
    provider,
