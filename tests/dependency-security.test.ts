...
test("security-sensitive runtime dependencies stay pinned to audited versions", () => {
  ...
  expect(desktopManifest.overrides?.["js-yaml"]).toBe("4.3.1")
  expect(desktopLockfile).toContain('"js-yaml": ["js-yaml@4.3.1"')
  expect(desktopLockfile).not.toMatch(/"js-yaml": \["js-yaml@(?!4\.3\.1)/u)
})
