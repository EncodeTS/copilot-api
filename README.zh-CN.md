...
- **原生 Responses 异步工具：** 原生 `/responses` 路径会保留客户端声明的 `async` 和返回调用项的异步标记，流式 item ID 修正也不会丢弃这些字段。跨轮回传完整历史时，未完成调用和稍后交付的结果保留原始 `call_id`。工具执行与结果交付由 Codex 等客户端负责；gateway 不会自动把同步工具改成异步，不需要新增开关。Copilot HTTP 请求应显式发送对话历史：实测上游 HTTP 拒绝 `previous_response_id`。上游 WebSocket 支持续接，不代表本地提供了下游 WebSocket API 或 steering 支持。协议说明见 [OpenAI async tool calling](https://developers.openai.com/api/docs/guides/async-tool-calling)。
