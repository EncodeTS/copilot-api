...
- **Native Messages outbound admission:** Built-in Copilot `/v1/messages` generation and `/v1/messages/count_tokens` serialize once, measure the final UTF-8 JSON body and canonical `image.source.data` carrier bytes, and send that checked string unchanged. The default is observe-only because no fresh dated Copilot boundary probe has replaced the older empirical evidence; diagnostics contain numeric sizes and decisions only. Provider routes do not inherit this profile.
  - `nativeMessagesOutboundMaxBodyBytes` and `nativeMessagesOutboundMaxImageSourceDataBytes` optionally set endpoint-specific byte limits. Neither has a default until a fresh redacted canary establishes one.
  - `nativeMessagesOutboundHardEnforcement` defaults to `false`. Setting it to `true` enforces whichever explicit limits are configured; setting it back to `false` is the rollback control. Enabling it without either limit remains observe-only.
...
- **Native Responses async tools:** The gateway preserves client-provided `async` declarations and returned call metadata on the native `/responses` path, including through stream item-ID normalization. Pending calls and later results retain their original `call_id` across full-history requests. The client, such as Codex, owns job execution and result delivery; synchronous tools are not automatically made asynchronous. No gateway switch is required. For Copilot HTTP requests, send conversation history explicitly: the tested upstream rejects `previous_response_id` over HTTP. Upstream WebSocket continuation support does not imply a downstream WebSocket API or steering support. See [OpenAI async tool calling](https://developers.openai.com/api/docs/guides/async-tool-calling).
- **Copilot protocol differences:** Direct GPT-6 probes reject `service_tier` even for `auto` and `default`, so the compatibility stripping remains. Copilot also rejects image `detail: "original"`; its native route maps this to `high` without changing image bytes, and its Codex model catalog disables the original-detail capability. HTTP responses preserve allowed upstream request/retry/rate-limit metadata. Unsequenced Copilot error events terminate the stream without a second synthetic EOF error. See the [validation report](COPILOT_PROTOCOL_VALIDATION.md).
...
| Endpoint                                                    | Method | Description                                                                                                                                                                                                                           |
| ----------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /alpha/search`<br>`POST /v1/alpha/search`             | `POST` | Compatibility aliases that transparently forward the exact JSON body and query string to the Codex Alpha Search upstream.                                                                                                             |
...
# For a configured openai-responses provider named "openai"
